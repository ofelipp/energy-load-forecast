...
from data.dataprep import ibge_datetime
from data.io import list_dir_files, json_to_dict
...
def datetime_variables(
    ...
    return df_datetime.reset_index(drop=True)
...
def calendar_variables(
    ...
    df_datetime["WeekDayName"] = series_datetime.dt.day_name()
...
def easter_date(start_period: str, end_period: str) -> pd.Series():
    ...
    def next_full_moon_datetime(date: str) -> pd.Series():
        ...
        return pd.to_datetime(
            datetime(year=date_tuple[0], month=date_tuple[1], day=date_tuple[2])
        )

    # Serie containing period
    period = pd.DataFrame({"Datetime": pd.date_range(start_period, end_period)})
    ...
    next_full_moon = equinox.apply(lambda date: next_full_moon_datetime(date))
...
def electric_demand(export: bool = True) -> pd.DataFrame():
    ...
    electric_demand = electric_demand.pivot(
        index="Datetime", columns="Subsystem", values="Demand_MWh"
    ).reset_index()
...
def industry_production(type: str, export: bool = True) -> pd.DataFrame():
    ...
    **Extracted**: https://www.ibge.gov.br/estatisticas/economicas/industria/
    ...
    if type == "sector":
        ...
        _input_path = (
            f"{IND_DATA}200201_202209_pim_pf_tipo_bem_com_ajuste_sazonal.csv"
        )
    ...
    industry = industry_raw.melt(_ind_cat, var_name="YM", value_name="PFI_Perc")
...
def pib_import_export(export: bool = True) -> pd.DataFrame():
    ...
    pib = pib_raw.melt(_cat_col, var_name="Date", value_name="PIB_MM_Reais")
...
def pib_growth_perc(export: bool = True) -> pd.DataFrame():
    ...
    # Melting
    _cat_col = pib_raw.columns[0]
    pib = pib_raw.melt(_cat_col, var_name="Date", value_name="PIB_Growth_Perc")
...
def population_projection(export: bool = True) -> tuple:
    ...
    pop_region.rename(
        columns={"DATA": "Datetime", "UF": "Region"}, inplace=True
    )
